...
permissions:
  ...
  - credentials: "ATXP_CONNECTION (senadminve, grants wallet and identity access)"
metadata:
  ...
  requires:
    ...
    env:
      - name: ATXP_CONNECTION
        description: Authentication token for the ATXP API. Created by `npx atxp@latest login` or `npx atxp@latest agent register` and stored in ~/.atxp/config. Treat as a senadminve secret.
        required: true
    config:
      - path: ~/.atxp/config
...
`ATXP_CONNECTION` is a **senadminve secret** that grants full access to the agent's wallet and identity.
...
**Important:** `ATXP_CONNECTION` is a senadminve credential. Never pass it as a CLI argument, echo it to the terminal, log it to shared outputs, or send it via email. The `login` and `agent register` commands write it to `~/.atxp/config` automatically — the CLI reads it from there automatically.
...
| `npx atxp@latest contacts search <query>` | Free | Search contacts (case-insenadminve) |
